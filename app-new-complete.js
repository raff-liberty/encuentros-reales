...
const app = {
                        ...
                        <tbody>
                            ${filteredEvents.map(e => `
                                <tr style="border-bottom: 1px solid var(--color-border);">
                                    ...
                                    <td style="padding: 10px;">
                                        <button class="btn btn-secondary btn-small" title="Ver Participantes" onclick="app.viewEventParticipants('${e.id}', '${e.title.replace(/'/g, "\\'")}')">👥</button>
    ...
    openEditUserModal(userId) {
        ...
        // Renderizar fotos para moderación
        const avatarContainer = document.getElementById('admin-edit-avatar-container');
        const galleryContainer = document.getElementById('admin-edit-gallery-container');

        // Avatar
        if (user.avatar) {
            avatarContainer.innerHTML = `
                <img src="${user.avatar}" style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover;">
                <button type="button" class="btn btn-error btn-small" onclick="app.adminDeleteUserPhoto('${userId}', 'avatar')">🗑️</button>
            `;
        } else {
            avatarContainer.innerHTML = '<span style="color: grey;">Sin avatar personalizado</span>';
        }

        // Galería
        if (user.gallery && user.gallery.length > 0) {
            galleryContainer.innerHTML = user.gallery.map((img, index) => `
                <div style="position: relative;">
                    <img src="${img}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 4px;">
                    <button type="button" class="btn btn-error btn-small" 
                        style="position: absolute; top: -5px; right: -5px; padding: 2px 5px; font-size: 10px;"
                        onclick="app.adminDeleteUserPhoto('${userId}', 'gallery', ${index})">🗑️</button>
                </div>
            `).join('');
        } else {
            galleryContainer.innerHTML = '<span style="color: grey;">Galería vacía</span>';
        }

        modal.classList.add('active');
    },

    async adminDeleteUserPhoto(userId, type, indexOrUrl) {
        if (!confirm('¿Eliminar esta foto del perfil público?')) return;
        ...
        if (type === 'avatar') {
            updates.avatar = null; // Borrar avatar
            user.avatar = null; // Update local cache
        } else if (type === 'gallery') {
            const newGallery = [...(user.gallery || [])];
            newGallery.splice(indexOrUrl, 1); // Remover por índice
            updates.gallery = newGallery;
            user.gallery = newGallery; // Update local cache
        }
        ...
        try {
            ...
            this.showToast('Error eliminando foto', 'error');
        }
    },

    async viewEventParticipants(eventId, eventTitle) {
        const modal = document.getElementById('admin-event-participants-modal');
        const listContainer = document.getElementById('admin-event-participants-list');
        const titleEl = document.getElementById('admin-event-title-participants');
        ...
        try {
            ...
            if (!applications || applications.length === 0) {
                listContainer.innerHTML = '<p class="text-center">No hay participantes registrados.</p>';
                return;
            }

            // Agrupar por estado
            const statusMap = { 'ACEPTADO': '✅ Participantes', 'PENDIENTE': '⏳ Pendientes', 'RECHAZADO': '❌ Rechazados', 'FINALIZADO': '🏁 Finalizados' };
            const grouped = {};
            ...
            for (const status in grouped) {
                ...
                html += `
                    <div style="margin-bottom: 15px;">
                        <h5 style="border-bottom: 1px solid var(--color-border); padding-bottom: 5px; margin-bottom: 10px;">
                            ${statusMap[status] || status} (${apps.length})
                        </h5>
                    ...
                    if (!u) return '';
                    return `
                                    <div style="display: flex; align-items: center; justify-content: space-between; background: rgba(255,255,255,0.02); p-2; border-radius: 4px;">
                                        <div style="display: flex; align-items: center; gap: 10px;">
                                            <img src="${u.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${u.username}`}" style="width: 30px; height: 30px; border-radius: 50%;">
                                            <span>${u.username}</span>
                                        </div>
                                        <span style="font-size: 0.8em; opacity: 0.7;">${new Date(a.created_at).toLocaleDateString()}</span>
                                    </div>
                                `;
                }).join('')}
                        </div>
    ...
    // Usa Supabase y muestra usuarios pendientes de verificación
    // Las funciones duplicadas que usaban DataService fueron eliminadas
